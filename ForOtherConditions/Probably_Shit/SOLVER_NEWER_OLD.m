                ...
                for iA_alpha_2 = 1:size(PitchAmp_2)
                    for iZeta = 1:size(zeta)
                        ...
                        for k = 1:NTS
                            ...
                            % ANGLE OF ATTACK 
                            alpha = AoA;% atan(V_INF(2)/V_INF(1));
                            %alpha_le = AMP_AL*cos(2*pi*freq*dt*k)-AMP_AL;
                            %alpha = -atan((pivot*sin(alpha_le)+(1-pivot)*sin(alpha_1))/(pivot*cos(alpha_le)+(1-pivot)*cos(alpha_1)));
                            %alpha = -(alpha_1+AMP_AL_2*sin(2*pi*freq*dt*k));
...
%                             plot(XZ,YZ,'o');
%                             hold on;
                            quiver(CPX,CPY,cprx,cpry,10,'black','filled');
                            hold on;
...
%                             pbaspect([2 1 1]);
                            ...
                            if rem(k,NTSPP) == 0 && floor(k/NTSPP) > 1
                                ...
                                fprintf('Timestep: %3d | meanCT: %2.5f | meanCP: %2.5f | Eta: %2.5f\n',k,meanCT,meanCP,ETA);
