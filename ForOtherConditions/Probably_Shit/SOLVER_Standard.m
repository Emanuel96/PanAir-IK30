                        ...
                        for k = 1:NTS
                            ...
                            for i = 1:N
                                ...
                                if i > iSLE && i < iELE
                                    ...
                                    OMEGA = -[0 0 -2*pi*freq*AMP_AL*cos(2*pi*freq*dt*k)]; % angle of attack is symetrical
                                    ROT(k) = OMEGA(3);
                                    ...
                                    OMEGA = -[0 0 -2*pi*freq*AMP_AL_2*cos(2*pi*freq*dt*k+pi/2)];
                                    ROT(k) = OMEGA(3);
                                    % ROTATIONAL VELOCITY AT AIRFOIL NODES
                                    vecpos_node = [x(i) y(i) 0];
                                    vel_node = cross(OMEGA,vecpos_node);
...
%                                 if abs(TAUM1-(-C2-RADI)/(2*C1)) < abs(TAUM1-(-C2+RADI)/(2*C1))
%                                     TAU = (-C2-RADI)/(2*C1);
%                                 else
%                                     TAU = (-C2+RADI)/(2*C1);
%                                 end
                                ...
                                if abs(VW - VW_old) <= 0.001 && abs(UW - UW_old) <= 0.001
                                    break;
                                end
...
%                                 quiver(0,0,0,CL(k),1,'blue','filled');
                            ...
                            if k == NTS-NTSPP/4
                                press_desc = fopen(strcat(folder_name,'/pressure_075.dat'),'w');
                                for airfoil_c = 1:N
                        ...
                        %fprintf(fid_force,'VARIABLES=t,y_dot,alpha_eff,CD,CL,CM,CPP,CPR\n');
                        for k = NTS-NTSPP:NTS % PRINT ONLY LAST PERIOD
                            fprintf(fid_force,"%f %f %f %f %f %f %f %f\n",k*dt/(1/freq)-(NPs-1),PLUNG_VEL(k),ALPHA_EFF(k),CD(k),CL(k),CM(k),-CD(k),-PLUNG_VEL(k)*CL(k)/V_mag);
                        end
                        fclose(fid_force);
