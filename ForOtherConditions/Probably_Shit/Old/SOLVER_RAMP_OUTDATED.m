...
%  Last Update: June 20th, 2022 by Emanuel Camacho
...
%fid = fopen(sprintf('Results_%0.2f_%0.2f.dat',NonDimAmpl,RedFreq),'w');
...
for k = 1:NTS
    ...
    for i = 1:N
        % RAMP MOTION
        HY = AMP/(1+exp(-RedFreq*(dt*k-x0)));
        V_XY_AIRFOIL(i,:) = [0 AMP*k*exp(-RedFreq*(dt*k-x0))/(1+exp(-RedFreq*(dt*k-x0)))^2];
...
%         meanCP = mean(plungingP(k-NTSPP:k));
%         %% PROPULSIVE EFFICIENCY
%         ETA = meanCT/meanCP;
%         
%         fprintf('Timestep: %3d | meanCT: %2.5f | meanCP: %2.5f | Theta: %2.5f\n',k,meanCT,meanCP,ETA);
%     end
end
