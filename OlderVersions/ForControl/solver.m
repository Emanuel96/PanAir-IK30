...
if isfile('Results/Results_for_NN.dat') == 1
    fparstu = fopen('Results/Results_for_NN.dat','a');
else
    fparstu = fopen('Results/Results_for_NN.dat','w');
    fprintf(fparstu,'Re h k Alpha CPP CPR ETA\n');
end
            ...
            for iRedFreq = 1:size(RedFreq)
                for iA_alpha = 1:size(PitchAmp)
                    for iZeta = 1:size(zeta)
                        ...
                        %precomputed_kinematics(); DEACTIVATED
