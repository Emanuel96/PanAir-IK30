function [BN,BT] = calc_BN_BT(x,y,N)
    for i = 1:N
        for j = 1:N
            if i == j
                BN(i,j) = 0;
                BT(i,j) = 0.5;
            else
                x_m = 0.5*(x(i)+x(i+1));
                y_m = 0.5*(y(i)+y(i+1));
                theta1 = atan2(y(i+1)-y(i),x(i+1)-x(i));
                theta2 = atan2(y(j+1)-y(j),x(j+1)-x(j));
                beta = atan2((y_m-y(j+1))*(x_m-x(j))-(x_m-x(j+1))*(y_m-y(j)),(x_m-x(j+1))*(x_m-x(j))+(y_m-y(j+1))*(y_m-y(j)));            
                r = ((x_m-x(j))^2+(y_m-y(j))^2)^0.5;
                rP1 = ((x_m-x(j+1))^2+(y_m-y(j+1))^2)^0.5;
                BN(i,j) = 1/(2*pi)*(cos(theta1-theta2)*log(rP1/r)-sin(theta1-theta2)*beta);
                BT(i,j) = 1/(2*pi)*(cos(theta1-theta2)*beta+sin(theta1-theta2)*log(rP1/r));
            end
        end
    end
end