...
pivot = 0.3;
% ONLY LEADING EDGE? 0 - STANDARD FLAPPING | 1 - LEADING-EDGE |
onlyLE = 0;
smoother = 117.7;
        ...
        for iRedFreq = 1:size(RedFreq)
            for iA_alpha = 1:size(PitchAmp)
                for iZeta = 1:size(zeta)
                    ...
                    fprintf('Re: %f | k: %2.3f | h: %2.3f | A: %2.3f | Mean AoA: %2.3f |\n',Re(iRe),RedFreq(iRedFreq),NonDimAmpl(iNonDimAmpl),-PitchAmp(iA_alpha)*180/pi,AoA*180/pi);
                    ...
                    for k = 1:NTS
                        ...
                        for i = 1:N
                            ...
                            % ALPHA
                            pos_m1 = AMP_AL*sin(2*pi*freq*(dt*k-dt));
                            pos_p1 = AMP_AL*sin(2*pi*freq*(dt*k+dt));
                        ...
                        ALPHA_1(k) = -(AMP_AL*sin(2*pi*freq*dt*k)-AoA); %POSITIVE UP
                        if onlyLE == 1
                    ...
                    %% CONDITION NUMBER
                    folder_name = sprintf('G:/My Drive/[AEROG] NeuroLift/Results/DataForTraining/%d',cond_no);
                    mkdir(folder_name);
                    %% REGIME FILE
