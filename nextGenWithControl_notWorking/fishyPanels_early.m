            ...
            for iA_alpha = 1:size(PitchAmp)
                %for iA_alpha_2 = 1:size(PitchAmp_2)
                    for iZeta = 1:size(zeta)
                        ...
                        fprintf('k: %2.3f | h: %2.3f | A: %2.3f\n',RedFreq(iRedFreq),NonDimAmpl(iNonDimAmpl),-PitchAmp(iA_alpha)*180/pi);
...
%                         %ALERT: (COORDINATES MUST ALWAYS BE CREATED/READ ON THE COUNTERCLOCKWISE DIRECTION)
%                         SD7003 = readmatrix('Airfoils_Coordinates/SD7003_REFINED.dat');
...
%                             pause(0.01);
                        ...
                        for k = 1:NTS
                            ...
                            %% PID BLOCK FOR AERODYNAMIC BEHAVIOR
