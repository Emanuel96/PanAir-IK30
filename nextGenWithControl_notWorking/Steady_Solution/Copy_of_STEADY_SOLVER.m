...
%     y = y_wd;
    ...
    x = x_wd*cos(alpha0)-y_wd*sin(alpha0);
    y = x_wd*sin(alpha0)+y_wd*cos(alpha0);
...
%     end
    ...
    CD = (CX*cos(AoA)+CY*sin(AoA))/chord;
    CL = (CY*cos(AoA)-CX*sin(AoA))/chord;
    CM = C_M/chord;
    
    %% CONTROL
    
    SP = 0.5*sin(2*pi*freq*k*dt);
    
    %SP = 0.2;
    
    error(k) = SP - CL;
    
    %alpha0 = alpha0 - 0.001*error(k);
    
    lift(k) = CL;
    alpha_pos(k) = AoA;
    
    alpha0 = alpha0 - 0.01*error(k);
