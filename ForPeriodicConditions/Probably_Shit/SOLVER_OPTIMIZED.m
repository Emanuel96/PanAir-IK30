...
%NonDimAmpl = 0.5;
NonDimAmpl = [0.1:0.1:1.0]';
% REDUCED FREQUENCY
%RedFreq = 0.05;
RedFreq = [0.01:0.01:0.07]';
            ...
            for iA_alpha = 1:size(PitchAmp)
                for iA_alpha_2 = 1:size(PitchAmp_2)
                    for iZeta = 1:size(zeta)
                        ...
                        folder_name = sprintf('G:/My Drive/[AEROG] NeuroLift/Results/DataForTraining/Re_%6.0f_h%0.3f_k%0.2f_A%0.2f',Re(iRe),NonDimAmpl(iNonDimAmpl),RedFreq(iRedFreq),-PitchAmp(iA_alpha)*180/pi);
