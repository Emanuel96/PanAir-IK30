...
% LINE 520. Is there an angle of attack?
...
alpha_1 = -0*pi/180;
% PITCHING AMPLITUDE
%PitchAmp = -[atan(0.5*0.1)*180/pi]'*pi/180;
PitchAmp = -[0]'*pi/180;
PitchAmp_2 = -[0]'*pi/180;
...
fprintf(fparstu,'VARIABLES=Re,h,k,Alpha,Alpha_2,CT,CP,ETA\n');

data_set = fopen('Results/DATA_SET.dat','w');
                ...
                for iA_alpha_2 = 1:size(PitchAmp_2)
                    for iZeta = 1:size(zeta)
                        ...
                        for k = 1:NTS
                            ...
                            ALPHA_EFF_dot = gradient(ALPHA_EFF)/dt;
                            ALPHA_EFF_dot_dot = gradient(ALPHA_EFF_dot)/dt;
                            ...
                            for iter = 1:NITER
                                ...
                                %                         if rem(iter,reportConvergence) == 0
...
%                             pause(0.01);
%                             quiver(CPX,CPY,cprx,cpry,1,'black','filled');
%                             hold on;
%                             quiver(0,0,0,-V_XY_AIRFOIL(i,2),3,'blue','filled');
%                             hold on;
%                             quiver(0,0,CX,CY,0.5,'red','filled');
%                             hold off;
%                             daspect([1 1 1]);
%                             pbaspect([2 1 1]);
                            
%                             pause(0.01);
%                             plot(x(1:N+1),y(1:N+1),'black','LineWidth',1);
%                             hold on;
%                             %scatter(x,y,10);
%                             quiver(CPX,CPY,cprx,cpry,2.5,'black','filled');
...
%                             pbaspect([2 1 1]);
                            ...
                            if rem(k,NTSPP) == 0 && floor(k/NTSPP) > 1
                                %% MEAN DRAG
                                meanCT = mean(-CD(k-NTSPP:k));
                                %% MEAN POWER
                                plungingP = -PLUNG_VEL.*CL/V_mag;
                                %CM = -CM; %VERIFY THIS
                                pitchingP = -ROT.*CM*chord/V_mag;
                                meanCP = mean(plungingP(k-NTSPP:k)+pitchingP(k-NTSPP:k));
                                %% PROPULSIVE EFFICIENCY
                                ETA = meanCT/meanCP;
                                
                                fprintf('Timestep: %3d | meanCT: %2.2f | meanCP: %2.2f | Eta: %2.2f\n',k,meanCT,meanCP,ETA);
                        ...
                        fid_force = fopen(sprintf('Results/Forces_h%0.3f_k%0.2f_A%0.2f.dat',NonDimAmpl(iNonDimAmpl),RedFreq(iRedFreq),PitchAmp(iA_alpha)*180/pi),'w');
                        for k = NTS-NTSPP:NTS % PRINT ONLY LAST PERIOD
                            fprintf(fid_force,"%f %f %f %f %f %f %f %f\n",k*dt/(1/freq),PLUNG_VEL(k),ALPHA_EFF(k),CD(k),CL(k),CM(k),CT(k),CPR(k));
                        end
                        fclose(fid_force);
...
%                         plot(XZ,YZ,'o');
                        ...
                        % APPROX
                        matrix_A = [ones(NTSPP+1,1) ALPHA_EFF(NTS-NTSPP:NTS) ALPHA_EFF_dot(NTS-NTSPP:NTS) ALPHA_EFF_dot_dot(NTS-NTSPP:NTS)];
                        matrix_B = CD(NTS-NTSPP:NTS);
                        MATRIX = inv(matrix_A'*matrix_A)*matrix_A';
                        COEFF = MATRIX*matrix_B;
                        R_squared = 1 - sum((CD(NTS-NTSPP:NTS)-matrix_A*COEFF).^2)/sum((CD(NTS-NTSPP:NTS)-mean(CD(NTS-NTSPP:NTS))).^2);
                        fprintf(data_set,'CD %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f\n',Re(iRe),NonDimAmpl(iNonDimAmpl),RedFreq(iRedFreq),-PitchAmp(iA_alpha)*180/pi,-PitchAmp_2(iA_alpha_2)*180/pi,COEFF(1),COEFF(2),COEFF(3),COEFF(4),R_squared);
                        CD_APPROX = matrix_A*COEFF;
                        
                        subplot(1,3,1);
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CD(NTS-NTSPP:NTS),'black','LineWidth',1);
                        hold on;
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CD_APPROX,'red','LineWidth',1);
                        xlabel('$\alpha_{eff}$','interpreter','latex','fontsize',15);
                        ylabel('$C_d$','interpreter','latex','fontsize',15,'rotation',0);

                        % APPROX
                        matrix_A = [ones(NTSPP+1,1) ALPHA_EFF(NTS-NTSPP:NTS) ALPHA_EFF_dot(NTS-NTSPP:NTS) ALPHA_EFF_dot_dot(NTS-NTSPP:NTS)];
                        matrix_B = CL(NTS-NTSPP:NTS);
                        MATRIX = inv(matrix_A'*matrix_A)*matrix_A';
                        COEFF = MATRIX*matrix_B;
                        R_squared = 1 - sum((CL(NTS-NTSPP:NTS)-matrix_A*COEFF).^2)/sum((CL(NTS-NTSPP:NTS)-mean(CL(NTS-NTSPP:NTS))).^2);
                        fprintf(data_set,'CL %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f\n',Re(iRe),NonDimAmpl(iNonDimAmpl),RedFreq(iRedFreq),-PitchAmp(iA_alpha)*180/pi,-PitchAmp_2(iA_alpha_2)*180/pi,COEFF(1),COEFF(2),COEFF(3),COEFF(4),R_squared);
                        CL_APPROX = matrix_A*COEFF;
                        
                        subplot(1,3,2);
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CL(NTS-NTSPP:NTS),'black','LineWidth',1);
                        hold on;
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CL_APPROX,'red','LineWidth',1);
                        xlabel('$\alpha_{eff}$','interpreter','latex','fontsize',15);
                        ylabel('$C_l$','interpreter','latex','fontsize',15,'rotation',0);
                        
                        % APPROX
                        matrix_A = [ones(NTSPP+1,1) ALPHA_EFF(NTS-NTSPP:NTS) ALPHA_EFF_dot(NTS-NTSPP:NTS) ALPHA_EFF_dot_dot(NTS-NTSPP:NTS)];
                        matrix_B = CM(NTS-NTSPP:NTS);
                        MATRIX = inv(matrix_A'*matrix_A)*matrix_A';
                        COEFF = MATRIX*matrix_B;
                        R_squared = 1 - sum((CM(NTS-NTSPP:NTS)-matrix_A*COEFF).^2)/sum((CM(NTS-NTSPP:NTS)-mean(CM(NTS-NTSPP:NTS))).^2);
                        fprintf(data_set,'CM %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f %2.5f\n',Re(iRe),NonDimAmpl(iNonDimAmpl),RedFreq(iRedFreq),-PitchAmp(iA_alpha)*180/pi,-PitchAmp_2(iA_alpha_2)*180/pi,COEFF(1),COEFF(2),COEFF(3),COEFF(4),R_squared);
                        CM_APPROX = matrix_A*COEFF;      
                        
                        subplot(1,3,3);
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CM(NTS-NTSPP:NTS),'black','LineWidth',1);
                        hold on;
                        plot(ALPHA_EFF(NTS-NTSPP:NTS),CM_APPROX,'red','LineWidth',1);
                        xlabel('$\alpha_{eff}$','interpreter','latex','fontsize',15);
                        ylabel('$C_m$','interpreter','latex','fontsize',15,'rotation',0);
